...
const NavBar = () => {
  ...
  const isLoggedIn =
    !PRE_APP_PATHS.includes(pathname) && !PRE_APP_PATHS.includes(pathname)
  ...
  return !OUTER_APP_PATHS.includes(pathname) ? (
    <Container>
      <Inner>
        <LeftNavContainer>
          {isAdmin && (
            <Logo onClick={() => history.push('/onboarding-clients')} />
          )}
          {!isAdmin && (
            <Logo
              onClick={() => history.push(`/conversations/${currentUser._id}`)}
            />
          )}
